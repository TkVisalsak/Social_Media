...
export const createStory = TryCatch(async (req, res) => {
  ...
  const uploaded = await cloudinary.v2.uploader.upload(fileUrl.content, {
    ...
    folder: "stories", // namespace from posts
  });

  const story = await Story.create({
    userId: req.user._id,
    mediaUrl: [                          // ✅ matches schema field name
      {
...
export const viewStory = async (req, res) => {
  try {
    const userId = req.user._id;                // ✅ consistent: use _id not .id
...
export const deleteStory = async (req, res) => {
  try {
    ...
    const story = await Story.findOne({ _id: storyId, userId });  // ✅ userId not user

    if (!story) {
      return res.status(404).json({ message: "Not found or unauthorized" });
    }

    // delete from Cloudinary before removing doc
    for (const m of story.mediaUrl) {           // ✅ mediaUrl not media
      if (m.publicId) {
